...
//-------------------------------------------------------------------------------------------------
// 
// Method reading the text file (AsciiInput) and filling MARS particle object
//
//-------------------------------------------------------------------------------------------------

int MarsParticle::read(std::vector<std::string> *eventAsStringVector, double norm_factor, int type) 
{
  if(eventAsStringVector->size() != 17 && eventAsStringVector->size() != 20) 
  {
  ...
  // type = 0 : beam-gas inelastic
  // type = 1 : beam-gas elastic
  // type = 2 : beam-halo

  m_showerPrimary.processId = 100.*type + m_showerPrimary.processId;

  m_weight *= norm_factor;
