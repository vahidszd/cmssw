...
#
# Goal: 
# Creates a buffer containing events from MIB simulation, ordered with their initial weight
# The ordering is such that once the buffer has been written, if one picks up randomly an event into
# it, it's final weight will be one.
#
# Using this, the events generated from the buffer are unweighted, and it makes rates calculations
...
BeamHaloParticleBuffer::BeamHaloParticleBuffer(std::string fileName,
					       CLHEP::HepRandomEngine* engine): m_fileName(fileName),
										...
										m_writeFlag(false) {
  // The event size will be always the same (written in a binary file) 
  m_recordSize           = sizeof(long)*2 + sizeof(double)*8;
...
int BeamHaloParticleBuffer::writeParticle(BeamHaloParticle *particle) 
{
  ...
  long pdgId = 0, origin = 0;
  double px = 0., py = 0., pz = 0., e = 0., x = 0., y = 0., z = 0., weight = 0.;
  ...
  origin = particle->processOrigin();

  m_ofstream.write((char*)(&pdgId), sizeof(long)); if(m_ifstream.bad()) return 0;
  m_ofstream.write((char*)(&origin), sizeof(long)); if(m_ifstream.bad()) return 0;
  ...
  m_intervalUpperBinEdge += weight; // Total weight of bin
  ...
  m_upperBinEdge += weight; // Total weight
...
BeamHaloParticle* BeamHaloParticleBuffer::readRandomParticle(void) 
{
  ...
  BeamHaloParticle *beamHaloParticle;
   ...
   // Look for the bin with the correct weight into total weight
   ...
   // Look for the particle with the correct weight into total weight of bin  
...
BeamHaloParticle* BeamHaloParticleBuffer::readParticle(void) 
{
  ...
  m_ifstream.read((char*)(&origin), sizeof(origin)); if(m_ifstream.bad()) return 0;
  ...
  HepMC::ThreeVector positionAtScoringPlane(x,y,z);
  BeamHaloParticle *beamHaloParticle = new BeamHaloParticle(pdgId, fourVector, positionAtScoringPlane, weight, origin);
